...
hints + last 3 Q/SQL pairs) → Groq JSON mode → `{sql, chart, explanation}` →
...
`tests/test_engine.py` (11 assertions) runs with a stubbed model and needs no API key.
...
path. `tests/benchmark.py` measures the scaling claim.
